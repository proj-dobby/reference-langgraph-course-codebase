...
def interrupt_handler(state: State) -> Command[Literal["llm_call", "__end__"]]:
    ...
    for tool_call in state["messages"][-1].tool_calls:
        ...
        #     if 'input' in raw_response.keys():
        #         response = raw_response['input']['resume'][0]
        #     elif 'Input' in raw_response.keys():
        #         response = raw_response['Input']['Resume'][0]
        #     else:
        #         raise Exception(f"Bad formatted Resume response recevied{type(raw_response)}: {raw_response}")
        # elif type(raw_response) == list:
        #     response = raw_response[0]
        # else:
        #     response = raw_response[0]
            # raise Exception(f"Bad formatted Resume response recevied - {type(raw_response)}: {raw_response}")
